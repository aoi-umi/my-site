...
export let detailQuery: MyRequestHandler = async (opt) => {
  ...
  if (!detail) throw common.error('', config.error.USER_NOT_FOUND);
...
export let update: MyRequestHandler = async (opt) => {
  ...
  let user = opt.myData.user;
  let updateCache: any = common.getDataInKey(data, [
    'avatar',
    'nickname',
    'profile',
  ]);
  ...
  if (updateCache.avatar)
    updateCache.avatarUrl = FileMapper.getImgUrl(
      updateCache.avatar,
      opt.myData.imgHost,
    );
...
export let oauthBind: MyRequestHandler = async (opt, ctx) => {
  ...
  await UserMapper.oauthBind(data, {
...
export let mgtDisable: MyRequestHandler = async (opt) => {
  ...
  let detail = await UserModel.findById(data._id);
  if (!detail.canEdit) throw common.error('不可禁用此账号');
  let update: any = {};
  let remark = '封禁';
  if (data.disabled) {
    if (data.disabledTo) {
