...
export let submit: MyRequestHandler = async (opt) => {
  ...
  if (user.equalsId(data.destUserId)) throw error('不能私信自己');
...
export let query: MyRequestHandler = async (opt) => {
  ...
  let rs = await ChatMapper.query(data, {
    userId: user._id,
  ...
  let rs = await ChatModel.aggregatePaginate(
    [
      ...
      },
      {
        $project: {
          key1: {
            $cond: [{ $eq: ['$userId', userId] }, '$userId', '$destUserId'],
          },
          key2: {
            $cond: [{ $ne: ['$userId', userId] }, '$userId', '$destUserId'],
          },
          data: '$$ROOT',
        },
      },
