...
export class Wx extends WxBase {
  ...
  getCodeUrl(type) {
    let uri = encodeURIComponent(
      `${env.host}/wx/auth?type=${type}&getUserInfo=1`,
    );
    let url =
  ...
  async getToken(data: { code }) {
    ...
    if (tokenRs.errcode) throw common.error(tokenRs.errmsg);
  ...
  async getUserInfo(data: { code: string }, opt?: { noReq?: boolean }) {
    ...
    if (cacheRs || opt.noReq) {
      if (opt.noReq && !cacheRs) throw common.error('已失效,请重新授权');
      return cacheRs;
    }
    ...
    if (rsData.errcode) throw common.error(rsData.errmsg);
    //token失效
