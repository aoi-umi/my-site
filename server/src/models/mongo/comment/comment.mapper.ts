...
export class CommentMapper {
  static async create(data: ValidSchema.CommentSubmit, type, user: LoginUser) {
    let lastComment = await CommentModel.findOne({
      ownerId: data.ownerId,
    }).sort({ floor: -1 });
    let lastMainComment = await CommentModel.findOne({
      ownerId: data.ownerId,
    }).sort({ mainFloor: -1 });
    let quote: CommentInstanceType;
    if (data.quoteId) quote = await CommentModel.findById(data.quoteId);
  ...
  static async resetComment<T = {}>(opt: {
    ...
    //获取二级回复
    if (getReply) {
      replyList = await CommentMapper.childQuery({
        replyTopId: commentList.map((ele) => ele._id),
      });
    }
    ...
    let userList = await UserMapper.queryById(userIdList, {
      imgHost: resetOpt.imgHost,
    });

    let voteList: VoteInstanceType[];
    if (resetOpt.user) {
      [voteList] = await Promise.all([
        //点赞
        VoteModel.find({
          userId: resetOpt.user._id,
          ownerId: allComment.map((ele) => ele._id),
        }),
      ]);
    }
    ...
    let rows = commentList.map((detail) => {
      ...
      if (getReply) {
        obj.replyList = allCommentList.filter((reply) =>
          detail._id.equals(reply.topId),
        );
      }
      return obj;
    });
  ...
  static resetDetail<T = {}>(detail, opt: CommentResetOption) {
    ...
    if (user) {
      if (voteList?.length) {
        let vote = voteList.find(
          (ele) =>
            ele.ownerId.equals(detail._id) && ele.userId.equals(user._id),
        );
    ...
    delete detail.__v;
    return detail as CommentDocType & {
      user: any;
      quoteUser: any;
      isDel: boolean;
    } & T;
  ...
  // user comment
  static async userCommentQuery(
    data: ValidSchema.UserCommentQuery,
    opt: {
      resetOpt?: CommentResetOption;
    },
  ) {
    ...
    let anyKeyAnd = BaseMapper.multiKeyLike(data.anyKey, (anykey) => {
      return {
        $or: [{ 'comment.comment': anykey }, { 'owner.title': anykey }],
      };
    });
    let and = [];
    if (anyKeyAnd.length) {
    ...
    if (data.isReply) {
      pipeline.push({
        $match: {
          $or: [{ 'owner.userId': userId }, { 'comment.quoteUserId': userId }],
        },
      });
    }
    ...
    if (data.isReply) {
      let quoteIds = rsRows
        .filter((ele) => ele.comment.quoteId)
        .map((ele) => ele.comment.quoteId);
