...
import {
  GoodsSpecGroupModel,
  GoodsSpecGroupInstanceType,
} from './goods-spec-group';
...
export class GoodsMapper {
  static async mgtSave(
    ...
    await transaction(async (session) => {
      ...
      await GoodsSpecGroupModel.insertMany(
        detail.specGroup.map((ele) => ele.toObject()),
        { session },
      );
      await GoodsSkuModel.insertMany(
        detail.sku.map((ele) => ele.toObject()),
        { session },
      );
    });
    return detail;
  ...
  static async detailQuery(
    data: ValidSchema.GoodsMgtDetailQuery,
    opt?: { normal?: boolean },
  ) {
    opt = {
      ...opt,
    ...
    if (!spu) throw error('', config.error.DB_NO_DATA);
  ...
  static async query(
    data: ValidSchema.GoodsMgtQuery,
    opt: {
      audit?: boolean;
      normal?: boolean;
      user: LoginUser;
  ...
  static async buy(data: ValidSchema.GoodsBuy, opt: { user: LoginUser }) {
    ...
    if (!sku) throw error('', config.error.DB_NO_DATA);
    if (sku.status !== myEnum.goodsSkuStatus.上架) throw error('商品已下架');
    ...
    if (totalPrice !== data.totalPrice)
      throw error('总价不一致', null, {
        remark: `c:${data.totalPrice};s:${totalPrice}`,
      });
    let { spu } = await GoodsMapper.detailQuery(
      { _id: sku.spuId },
      { normal: true },
    );
