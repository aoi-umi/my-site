...
export class GoodsSpu extends Base {
  ...
  @prop()
  get canUpdate() {
    return [myEnum.goodsStatus.下架, myEnum.goodsStatus.上架].includes(
      this.status,
    );
  }
  ...
  canView() {
    ...
    return (
      this.status === myEnum.goodsStatus.上架 &&
      this.putOnAt.getTime() <= now &&
      (!this.expireAt || this.expireAt.getTime() > now)
    );
  }
}

export const GoodsSpuModel = getModelForClass<
  GoodsSpu,
  typeof GoodsSpu & IPagination<GoodsSpu>
>(GoodsSpu);
