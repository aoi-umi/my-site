...
export class FavouriteMapper {
  static async create(opt: { ownerId; userId; type }) {
    let detail = await FavouriteModel.findOne({
      ownerId: opt.ownerId,
      userId: opt.userId,
    });
    if (!detail) {
  ...
  static async findOwner(opt: { ownerId; type }) {
    ...
    if (!owner) throw error('', config.error.NO_MATCH_DATA);
    return owner;
  }

  static lookupPipeline(opt: { userId: any; ownerIdKey?: string }) {
    return [
      {
        $lookup: {
  ...
  static async query(
    data: ValidSchema.FavouriteQuery,
