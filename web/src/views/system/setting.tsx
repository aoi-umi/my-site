    ...
    return (
      <MyLoad
        loadFn={async () => {
          ...
          if (rs.signUpFrom) {
            rs.signUpFrom = new Date(rs.signUpFrom)
          }
          if (rs.signUpTo) {
            rs.signUpTo = new Date(rs.signUpTo)
          }
      ...
      <div style={{ minHeight: '600px' }}>
        <Form label-position="top">
          ...
          <FormItem
            label="限定时间"
            v-show={[
              myEnum.settingSignUpType.限时关闭,
              myEnum.settingSignUpType.限时开放,
            ].includes(detail.signUpType)}
          >
            <DatePicker
              type="datetimerange"
              v-model={this.signUpTime}
              style={{ width: '300px' }}
            />
          </FormItem>
        </Form>
