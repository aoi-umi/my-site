...
export default class GoodsMgtDetail extends Base {
  ...
  async save() {
    await this.operateHandler(
      ...
      {
        beforeValid: () => {
          this.innerDetail.saveSku = this.sku
            .filter((ele) => this.skuStatusList.includes(ele.status))
            .map((ele) => {
        ...
        >
          ...
          <FormItem label="状态" prop="spu.status">
            <RadioGroup v-model={spu.status}>
              {myEnum.goodsStatus
                .toArray()
                .filter((s) => s.value !== myEnum.goodsStatus.已删除)
                .map((s) => {
          ...
          <Button
            ...
            on-click={() => {
              this.innerDetail.specGroup = helpers.clone(
                this.oldDetail.specGroup,
              )
          ...
          <FormItem label="sku" prop="saveSku"></FormItem>
          {this.renderSku()}
        </Form>
        <Button
          type="primary"
    ...
    specGroup[specIdx].value.forEach((ele) => {
      ...
      if (specIdx + 1 < specGroup.length) {
        ...
        if (this.skuResetType === myEnum.goodsResetType.规格值) {
          ...
          if (
            (this.skuResetType === myEnum.goodsResetType.规格数量 &&
              matchLength > 0) ||
            (this.skuResetType === myEnum.goodsResetType.初始化 &&
              matchLength === currSpec.length)
          ) {
  ...
  private setSkuCol() {
    const { specGroup } = this.innerDetail
    this.skuCol = [
      ...specGroup.map((ele, idx) => {
        return {
          title: ele.name,
          key: 'specGroup' + idx,
          render: (h, params) => {
            return <span>{this.sku[params.index].spec[idx]}</span>
          },
        }
      }),
      {
        ...
        key: 'price',
        render: (h, params) => {
          return <Input v-model={this.sku[params.index].price} type="number" />
        },
      },
      {
        title: '数量',
      ...
      },
      {
        ...
        render: (h, params) => {
          ...
          return (
            <Select
              v-model={this.sku[params.index].status}
              clearable
              placeholder="不设置"
            >
              {myEnum.goodsSkuStatus.toArray().map((ele) => {
                return (
                  <i-option value={ele.value} key={ele.value}>
                    {ele.key}
                  </i-option>
                )
              })}
  ...
  private renderSku() {
    return (
      <div class={this.getStyleName('sku-main')}>
        <label>
          <Checkbox v-model={this.skuShowSetOnly} />
          仅显示已设置
        </label>
        <Table
  ...
  private rowClassName(row, index) {
    if (!this.skuShowSetOnly || this.skuStatusList.includes(row.status)) {
      return ''
    }
    return 'hidden'
  }
