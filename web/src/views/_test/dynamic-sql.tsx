    ...
    dynamicComp.setConfigList([
      {
        ...
        queryMatchMode: {
          show: true,
          value: myEnum.dynamicCompStringQueryType.模糊,
        },
      },
      {
        name: 'testInt',
    ...
    return (
      <div>
        <Row gutter={5}>
          <Col span={12}>
            <Tabs>
              <TabPane label="sql">{this.renderSqlSetting()}</TabPane>
              <TabPane label="查询参数">
                <div>
                  <span>sql名字</span>
                  <Input v-model={this.querySqlName} />
                  <Input
                    v-model={this.querySqlOption}
                    type="textarea"
                    rows={3}
                  ></Input>
                </div>
          ...
          <Col span={12}>
            ...
            <Input type="textarea" v-model={this.result} rows={20}></Input>
          </Col>
        </Row>
      </div>
    )
  ...
  renderSqlSetting() {
    return (
      <Form label-position="right" label-width={60}>
        <FormItem label="参数">
          <Input type="textarea" v-model={this.params} rows={4}></Input>
        </FormItem>
        <Button
          on-click={() => {
            this.config.push({
