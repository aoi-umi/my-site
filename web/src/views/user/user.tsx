      ...
      <div>
        ...
        <Tabs animated={false} v-model={this.tab} class={this.getStyleName('tab')} on-on-click={(name: string) => {
          ...
          <TabPane name={myEnum.userTab.视频} label={() => {
            ...
            <Video ref={myEnum.userTab.视频} queryOpt={{ userId: detail._id }} notQueryOnRoute notQueryToRoute notQueryOnMounted />
          </TabPane>
          <TabPane name={myEnum.userTab.文章} label={() => {
            return <div>文章: {detail.article}</div>
          }}>
            <Article ref={myEnum.userTab.文章} queryOpt={{ userId: detail._id }} notQueryOnRoute notQueryToRoute notQueryOnMounted />
          </TabPane>
          <TabPane name={myEnum.userTab.粉丝} label={() => {
            return <div>粉丝: {detail.follower}</div>
          }}>
            <FollowList ref={myEnum.userTab.粉丝} userId={this.detail._id} followType={myEnum.followQueryType.粉丝} />
          </TabPane>
