...
import {
  ...
  DynamicCompSelectOptionsType,
  DynamicCompPropType,
  DynamicCompConfigFnType,
...
export class CompMgtDetail extends Vue<CompMgtDetailProp, Base> {
  ...
  protected created() {
    this.op = this.getOpModel({
      ...
      fn: async (type) => {
        ...
        if (['main', 'module', 'config'].includes(type)) {
    ...
    return (
      <div class={this.getStyleName('root')}>
        {this.itemOnly ? (
          ...
          <Tabs>
            <TabPane label="基本信息">{this.renderBaseInfo()}</TabPane>
            <TabPane label="配置信息" disabled={!this.hasDetail}>
              {this.renderModule()}
              {this.selectedModuleIdx >= 0 && (
                <div>
                  <Divider />
                  <div>当前模块: {this.selectedModule.name}</div>
                  <Button
                    on-click={() => {
                      this.op.run('config')
                    }}
                  ...
                  <Tabs>
                    <TabPane label="字段设置">{this.renderItem()}</TabPane>
                    <TabPane label="按钮设置">
  ...
  private get selectedModule() {
    if (this.selectedModuleIdx >= 0) {
      return this.moduleList[this.selectedModuleIdx]
    }
  }
        ...
        onChange: ({ data }) => {
          if (data === this.selectedModule) {
            this.itemList.forEach((ele) => {
              if (
                this.selectedModule.viewType === dynamicCompViewType.查询条件
              ) {
                if (ele.queryMatchMode) {
                  ele.queryMatchMode.show = true
                }
              } else {
                if (ele.queryMatchMode) {
                  ele.queryMatchMode.show = false
                }
              }
            })
          }
              ...
              render: (h, params) => {
                return (
                  <div class={this.getStyleName('op')}>
                    ...
                    <a
                      on-click={() => {
                        if (this.selectedModuleIdx === params['index']) {
  ...
  selectOptionValChange(event) {
    try {
      ...
      if (
        this.selectConfig.optionType === DynamicCompSelectOptionsType.extraValue
      ) {
        this.selectOption = val
      } else {
        this.selectOption = JSON.parse(val)
      }
  ...
  private async configSave() {
    let itemValid = await Utils.valid(
      { data: this.itemList },
      {
        data: {
          type: 'array',
    ...
    let buttonValid = await Utils.valid(
  ...
  protected renderItem() {
    return (
      <Row class={this.getStyleName('config')} gutter={5}>
        ...
        <Col xs={12}>{this.renderSetting()}</Col>
      </Row>
    )
  }
    ...
    return (
      ...
      >
        <FormItem label="名字" prop="name">
          <Input
            v-model={this.selectConfig.name}
            on-on-change={() => {
        ...
        <FormItem label="显示文本" prop="text">
          <Input v-model={this.selectConfig.text}></Input>
        </FormItem>
        <FormItem label="类型">
        ...
        {[dynamicCompType.日期, dynamicCompType.日期时间].includes(
          this.selectConfig.type,
        ) && (
          <FormItem label="范围">
        ...
        {[dynamicCompType.选择器].includes(this.selectConfig.type) && (
          <div>
            ...
            <FormItem>
              <div>
                自定义为json格式：
                {[{ Obj选项: 'obj' }, [{ label: '列表选项', value: 'list' }]]
            ...
            <FormItem label="取值">
              <Input
                v-model={this.selectConfig.options}
                on-on-change={(val) => {
