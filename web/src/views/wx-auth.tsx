...
export default class WxAuth extends Base {
  ...
  getCodeUrl(data: { type: string; token?: string }) {
    const uri = encodeURIComponent(
      `${location.origin}/wx/auth?type=${data.type}&getUserInfo=1&token=${
        data.token || ''
      }`,
    )
    const url =
  ...
  render() {
    ...
    return (
      <Card class={this.getStyleName('user-info-card')}>
        ...
        <div
          class={this.getStyleName('qrcode-box')}
          v-show={
            !Utils.isWxClient() && (this.qrcodeLoginShow || this.qrcodeBindShow)
          }
        >
          <div>使用微信扫一扫</div>
      ...
      <div class={this.getStyleName('user-info')}>
        ...
        <div class={this.getStyleName('op')}>
          {this.accountChecking ? (
            ...
            <SignUp
              account={this.wxUserInfo.nickname}
              by={myEnum.userBy.微信授权}
              byVal={this.val}
            />
          )}
