...
export default class Pay extends Base {
    ...
    list.setQueryByKey(
      {
        ...query,
        createdAt: [
          query.createdAtFrom ? new Date(query.createdAtFrom) : '',
          query.createdAtTo ? new Date(query.createdAtTo) : '',
        ],
      },
  ...
  private updateRow(detail, rs) {
    ...
    data.splice(
      data.findIndex((ele) => ele._id === detail._id),
      1,
      {
        ...detail,
        ...rs,
    ...
    if (this.storeUser.user.hasAuth(authority.payMgtQuery)) {
      columns.push({
        ...
        render: (h, params) => {
          ...
          return <UserAvatar user={detail.user} style={{ margin: '5px' }} />
        },
      })
    }
    columns = [
      ...columns,
      {
        ...
        render: (h, params) => {
          const detail = params.row
          return this.storeUser.user.hasAuth(authority.payMgtQuery) ? (
            <a
              on-click={() => {
                this.goToPage({
      ...
      },
      {
        ...
        render: (h, params) => {
          const detail = params.row
          return <span>{this.$utils.dateFormat(detail.createdAt)}</span>
        },
      },
      {
        title: '操作',
