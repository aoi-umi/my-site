...
export class SideMenu extends Vue<SideMenuProp, Base> {
  ...
  private findCfg(data: MenuConfig[], path: string, matchLen = 0) {
    ...
    for (const ele of data) {
      ...
      if (ele.children) {
        const { name: childName, openNames: childOpenNames } = this.findCfg(
          ele.children,
          path,
          matchLen,
        )
  ...
  private isMenuItemShow(ele: MenuConfig) {
    ...
    if (show && ele.children) {
      const childShowLen = ele.children
        .map((child) => this.isMenuItemShow(child))
        .filter((s) => s)
  ...
  renderMenu() {
    return this.filterMenu(this.menuCfg).map((data) => {
      ...
      <Submenu class={this.getStyleName('sub-menu')} name={name}>
        <template slot="title">
          <Tooltip
            ...
            class={this.getStyleName('tooltip')}
            content={data.text}
            placement="right"
            transfer
          >
        ...
        {this.filterMenu(data.children).map((ele) =>
          this.renderMenuItem(ele, true),
        )}
      </Submenu>
    )
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {!this.isSmall ? (
          <Content
            class={this.getStyleName('blank')}
            style={{
              flex: `0 0 ${
                this.isCollapsed ? collapsedWidth : this.siderWidth
              }px`,
            }}
          ></Content>
        ) : (
