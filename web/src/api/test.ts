...
export class TestApi extends ApiModel<TestMethod> {
  constructor(apiConfig: TestApiConfigType) {
    ...
    this.fileUploadByChunksUrl = this.getRequestConfig(
      this.apiConfig.method.fileUploadByChunks,
    ).url
    this.fileUploadCheckUrl = this.getRequestConfig(
      this.apiConfig.method.fileUploadCheck,
    ).url
    this.imgUploadUrl = this.getRequestConfig(
      this.apiConfig.method.imgUpload,
    ).url
    this.imgUrl = this.getRequestConfig(this.apiConfig.method.imgGet).url
    this.videoUploadUrl = this.getRequestConfig(
      this.apiConfig.method.videoUpload,
    ).url
  ...
  uploader(file, url, opt?) {
    ...
    let result = this.requestByConfig<FileUploadRes>(url, {
      ...
      onUploadProgress: (progress) => {
        obj.progress = Math.round((progress.loaded / progress.total) * 100)
      },
    })
    obj.result = result
    return obj
