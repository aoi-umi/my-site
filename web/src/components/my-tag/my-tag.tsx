    ...
    return (
      <div class={this.getStyleName('root')}>
        {list.map((ele) => {
          ...
          if (ele.disabled) {
            return (
              <Tag class={this.getStyleName('disabled')} color="default">
                {ele.tag}
              </Tag>
            )
          }
          return (
            <Tag
              color={(ele.color as any) || 'blue'}
              checkable={ele.checkable}
              checked={ele.checked}
            ...
            >
              {ele.isDel ? <del>{ele.tag}</del> : ele.tag}
            </Tag>
          )
        })}
      </div>
...
export class MyTag<T = RenderTagType> extends Vue<
  MyTagBaseProp<T>,
  MyTagBase
> {}
